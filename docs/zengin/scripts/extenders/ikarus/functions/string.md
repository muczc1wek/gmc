...
The function returns a substring, if the starting position is invaid an empty string is returned.
...
The function returns an integer value of the string, if a string is invalid (doesn't contains an integer) zero is returned.
...
The function returns a substring at the specified offset after splitting the input string. If the offset is greater than or equal to the number of parts generated by splitting, the an empty string is returned.
